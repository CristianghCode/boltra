...
@Entity
@Table(name = "BTW_BT_FORM_CONT_TRIBUNAL")
public class FormacionContinuadaTribunal {
	
	@Id
	@GeneratedValue(strategy = GenerationType.SEQUENCE, generator = "sec_form_cont_tribunal")
	@SequenceGenerator(
			name="sec_form_cont_tribunal", sequenceName = "BOLTRA.SEQ_FORM_CONT_TRIBUNAL",
		    allocationSize = 1
		)
	@Column(name = "ID")
	private Long id;
	...
	@OneToOne
	@JoinColumn(name = "ID_FORMACION_CONTINUADA")
	private FormacionContinuada formacionContinuada;
	...
	public FormacionContinuada getFormacionContinuada() {
		return formacionContinuada;
	}

	public void setFormacionContinuada(FormacionContinuada formacionContinuada) {
		this.formacionContinuada = formacionContinuada;
	}
	...
	public int hashCode() {
		...
		result = prime * result + ((formacionContinuada == null) ? 0 : formacionContinuada.hashCode());
	...
	public boolean equals(Object obj) {
		...
		if (getClass() != obj.getClass())
			return false;
		FormacionContinuadaTribunal other = (FormacionContinuadaTribunal) obj;
		...
		if (fechaInicio == null) {
			...
			return false;
		if (formacionContinuada == null) {
			if (other.formacionContinuada != null)
				return false;
		} else if (!formacionContinuada.equals(other.formacionContinuada))
			return false;
		if (id == null) {
			if (other.id != null)
				return false;
